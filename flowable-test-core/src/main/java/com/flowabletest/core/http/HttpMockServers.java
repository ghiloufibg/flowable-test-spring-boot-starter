 ...
 * Spring's {@code @Autowired} resolution treats an injection point of generic type
 * {@code Map<String, X>} as "collect every bean of type X, keyed by bean name" rather than
 * "find the one bean whose own type happens to be that Map" -- so a directly-exposed Map bean
 * is never found by consumers autowiring that generic type. Wrapping it sidesteps that Spring
 * behavior entirely.
 */
