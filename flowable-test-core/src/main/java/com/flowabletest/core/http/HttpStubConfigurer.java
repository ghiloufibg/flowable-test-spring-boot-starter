 ...
 * echo something computed at test-startup time). Implement this on a test class and it is invoked
 * once per discovered service, after the plain JSON mapping files under that service's folder have
 * already been loaded -- so most implementations only need to add one dynamic stub on top of the
 * declarative ones, not rebuild the whole set.
 *
