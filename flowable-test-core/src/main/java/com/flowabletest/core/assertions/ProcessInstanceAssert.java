 ...
 * rather than a bare static factory, since evaluating these assertions requires the consumer's
 * own {@code RuntimeService}/{@code HistoryService} beans.
 */
public final class ProcessInstanceAssert extends AbstractAssert<ProcessInstanceAssert, String> {
    ...
    public ProcessInstanceAssert(String processInstanceId, RuntimeService runtimeService, HistoryService historyService) {
        super(processInstanceId, ProcessInstanceAssert.class);
        this.runtimeService = runtimeService;
        this.historyService = historyService;
    }

    /** The process instance has no active execution left, and history shows it reached {@code activityId}. */
    public ProcessInstanceAssert hasEndedAt(String activityId) {
        ...
        long active = runtimeService.createProcessInstanceQuery()
                ...
                .count();
        if (active != 0) {
            failWithMessage("Expected process instance <%s> to have ended, but it is still active", actual);
        }

        boolean reachedActivity = !historyService.createHistoricActivityInstanceQuery()
                .processInstanceId(actual)
                .activityId(activityId)
                .list()
                .isEmpty();
        if (!reachedActivity) {
            failWithMessage("Expected process instance <%s> to have reached end activity <%s>, but it did not",
                    actual, activityId);
        }
        return this;
    }
    ...
    public ProcessInstanceAssert isActive() {
        ...
        long active = runtimeService.createProcessInstanceQuery()
                ...
                .count();
        if (active == 0) {
            failWithMessage("Expected process instance <%s> to still be active, but it has ended", actual);
        }
        return this;
    }
    ...
    public ProcessInstanceAssert hasNoTaskForCandidateGroup(String candidateGroup) {
        ...
        long historicTasks = historyService.createHistoricTaskInstanceQuery()
                .processInstanceId(actual)
                .taskCandidateGroup(candidateGroup)
                .count();
        if (historicTasks != 0) {
            failWithMessage("Expected process instance <%s> to have no task for candidate group <%s>, but found %d",
