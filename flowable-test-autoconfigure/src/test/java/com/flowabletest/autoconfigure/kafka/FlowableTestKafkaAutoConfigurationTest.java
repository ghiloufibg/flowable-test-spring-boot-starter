...
/**
 * Proves the whole Kafka capability end to end (design doc section 4.2): the
 * {@code EnvironmentPostProcessor} discovers topics from the real {@code *.channel} fixtures
 * under {@code src/test/resources/channel-fixtures}, starts a broker, injects
 * {@code spring.kafka.bootstrap-servers} before context refresh, and
 * {@link FlowableTestKafkaAutoConfiguration} exposes both the broker and a working
 * {@link KafkaTestBridge} as beans -- with no {@code @EmbeddedKafka(topics = ...)} anywhere in
 * this test.
 */
...
class FlowableTestKafkaAutoConfigurationTest {
    ...
    void kafkaTestBridgeCanSendAndAwaitAMessage() {
        ...
        String received = kafkaTestBridge.awaitMessage(
