 ...
 * exposed by {@link FlowableTestKafkaAutoConfiguration} is a JVM-wide singleton shared across
 * every Spring context ({@code shared}, the default) or a fresh broker started per Spring context
 * ({@code per-context}). {@link #isShared(Environment)} throws {@link IllegalStateException} for
 * any other value, and is also called directly -- not only through {@link Condition} -- from
 * {@link FlowableTestKafkaEnvironmentPostProcessor} and {@link
 ...
 * PerContext}, so the corresponding {@code @Bean} methods remain mutually exclusive {@code
 * @Conditional} gates rather than a single method branching internally.
 */
