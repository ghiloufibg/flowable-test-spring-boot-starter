 ...
 *
 * <p>H2 is the default and needs no help from this class -- Spring Boot's own {@code
 * DataSourceAutoConfiguration} already wires an embedded H2 {@code DataSource} automatically
 * whenever H2 is on the test classpath and no explicit {@code spring.datasource.url} is configured.
 * This class only adds the opt-in {@code embedded-postgres} provider, for projects whose delegates
 * rely on Postgres-specific SQL (JSON columns, arrays) that H2's dialect emulation can't cover -- a
 * Docker-free replacement for a Testcontainers {@code PostgreSQLContainer}, activated via {@code
...
public class FlowableTestDatasourceAutoConfiguration {
  ...
  @ConditionalOnProperty(
      prefix = "flowable.test.datasource",
      name = "provider",
      havingValue = "embedded-postgres")
