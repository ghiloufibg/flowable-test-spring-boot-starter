 ...
 * {@code per-context} (the default) forks a fresh native process per Spring context, {@code
 * shared} starts at most one process per JVM and provisions a fresh logical database per context
 * on top of it. {@link #isShared} throws {@link IllegalStateException} for any other value.
 *
 * <p>Exposed as two nested {@link Condition} implementations, {@link PerContext} and {@link
 * Shared}, so the corresponding {@code @Bean} methods remain mutually exclusive {@code
 * @Conditional} gates rather than a single method branching internally -- the same shape used by
 * {@link EmbeddedPostgresPreferredCondition}.
 */
