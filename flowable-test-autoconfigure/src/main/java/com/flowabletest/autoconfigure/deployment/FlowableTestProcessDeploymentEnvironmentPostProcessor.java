...
/**
 * Resolves {@code flowable.test.processes.deploy} (optional, opt-in) into the authoritative default
 * set of BPMN processes to deploy, replacing Flowable's own {@code checkProcessDefinitions}
 * classpath scan. Absent (the default), this is a no-op: Flowable's own scan deploys every file
 * under {@code processDefinitionLocationPrefix}/{@code -Suffixes} exactly as today.
 *
 ...
 * <flowable.test.processes.root>/<name>.bpmn20.xml} (default root {@code classpath:processes}) via
 * {@link ProcessDeploymentDiscovery}, and {@code flowable.check-process-definitions=false} is
 * injected via {@code addFirst} (winning even over a consumer's own {@code application.yml}) so
 * this starter's allow-list becomes the sole mechanism deciding the default deployment set, rather
 * than racing Flowable's own scan for every file it can still find under the same root. Each
 * declared name is the BPMN <b>file</b> name (e.g. {@code "order-processing"}), not the {@code
 ...
 * property via {@link ProcessDeploymentRegistry#encode} for {@link
 * FlowableTestProcessDeploymentAutoConfiguration} to read back once {@code RepositoryService}
 * exists -- deployment itself can't happen here, since this runs before any bean exists.
 */
