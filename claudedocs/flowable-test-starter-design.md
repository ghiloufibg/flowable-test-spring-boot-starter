...
- `flowable.test.datasource.embedded-postgres.instance-scope` (`per-context` default | `shared`) additionally controls *how* the embedded-postgres provider allocates its server process, layered underneath the choice above — see `claudedocs/embedded-postgres-instance-scope-design.md` for the full design. `per-context` forks a fresh native process per Spring context (unchanged, original behavior); `shared` starts at most one process per JVM (`EmbeddedPostgresSupport`, mirroring the Kafka/WireMock JVM-wide-singleton pattern) and provisions a fresh, isolated logical database per context via `CREATE DATABASE` on top of it.
